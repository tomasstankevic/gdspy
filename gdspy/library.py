...
class Cell(object):
    ...
    def get_polygons(self, by_spec=False, depth=None, include_paths=True):
        """
            ...
            if by_spec:
                ...
                for reference in self.references:
                    if depth is None:
                        ...
                        next_depth = depth - 1
                    cell_polygons = reference.get_polygons(True, next_depth, include_paths)
                ...
                for reference in self.references:
                    if depth is None:
                        ...
                        next_depth = depth - 1
                    polygons.extend(reference.get_polygons(depth=next_depth, include_paths=include_paths))
        return polygons
...
class CellReference(object):
    ...
    def get_polygons(self, by_spec=False, depth=None, include_paths=True):
        ...
        if by_spec:
            polygons = self.ref_cell.get_polygons(True, depth, include_paths=include_paths)
            ...
            polygons = self.ref_cell.get_polygons(depth=depth, include_paths=include_paths)
...
class CellArray(object):
    ...
    def get_polygons(self, by_spec=False, depth=None, include_paths=True):
        ...
        if self.x_reflection:
            xrefl = numpy.array((1, -1))
        if by_spec:
            cell_polygons = self.ref_cell.get_polygons(True, depth, include_paths=include_paths)
            polygons = {}
            for kk in cell_polygons.keys():
                polygons[kk] = []
                for ii in range(self.columns):
            ...
            cell_polygons = self.ref_cell.get_polygons(depth=depth, include_paths=include_paths)
            polygons = []
            for ii in range(self.columns):
                for jj in range(self.rows):
                    spc = numpy.array([self.spacing[0] * ii, self.spacing[1] * jj])
